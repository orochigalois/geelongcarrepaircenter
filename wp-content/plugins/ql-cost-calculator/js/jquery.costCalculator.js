		...
		calculate : function(options){
			return this.each(function(){
				...
				if(options.formula.indexOf("power")>=0)
				{
					...
					//math power
					tempFormula = tempFormula.replace("{powerstart}", "Math.pow(");
					tempFormula = tempFormula.replace("{powerend}", ")");
				...
				$(this).html((options.currencyPosition=="before" ? '<span class="cost-calculator-currency">' + options.currency + '</span>' : '')+sum.toFixed(2).replace(".", options.decimalSeparator).replace(regex, '$&' + options.thousandthSeparator)+(options.currencyPosition=="after" ? '<span class="cost-calculator-currency">' + options.currency + '</span>' : ''));
				if(jQuery.type(options.updateHidden)=="object")
				{
					...
					$("#" + options.updateHidden.attr("id") + "-value").val(sum.toFixed(2));
				}
			});
		}
	};
