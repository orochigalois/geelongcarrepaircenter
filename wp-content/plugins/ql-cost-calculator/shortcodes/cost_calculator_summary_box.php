...
function cost_calculator_summary_box_shortcode($atts)
{
	...
	$output = '<div class="cost-calculator-box cost-calculator-summary-box clearfix' . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . (!empty($icon) && $icon!="none" ? ' ' . esc_attr($icon) : '') . '">
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				...
				"param_name" => "formula",
				"value" => "",
				"description" => __("Please put here the calculate formula for your form using the form elements ids. Available operators: + and *. Example: square-feet*walls+square-feet*floors", 'cost-calculator')
			),
			array(
				"type" => "textfield",
				"class" => "",
